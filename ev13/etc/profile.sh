...
physpathcmd1="${physpathcmd1:-realpath}"         ##--cmd or func to get physical-/real-path of symlinks (for now a cmd, but could be replaced by a func!); basic one: could do 'cd xx; pwd', but if xx not exist, breakes the script! but could be done in evv-syys! if this cmd not-exist in host-OS, alias this to something (cd?) in setevv.sh, so ...!
...
q_EvvDN="${q_EvvDP##*/}"
q_EvvDPPhys="$($physpathcmd1 $q_EvvDP)" ##--physical/real-absolute-path of evvDP, only for curr-env-ID  used ! not using1, 1OK:   "$(cd  $q_EvvDP && pwd -P)" 
...
[[ -r  $q_dvl1ProfFP         ]]  &&  source  $q_dvl1ProfFP        ;
...
HISTSIZE=1000  #--!-: this defines initial-read-items-numbers from histfile into the cu-shell-memory when a shell starts + AND also max-cmds-keep-in-histLIST at the end to write in histfile ! applies ONLY to the cu-shell/terminal-listing in RAM (but not file!)
