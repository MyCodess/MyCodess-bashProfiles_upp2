...
q_dvl1ProfFP=${q_EttcDP}/${q_dvl1ProfFN}
...
w1DPPhys="$($physpathcmd1 $w1DP)"  ##--physica-path-of-w1 for tars,.... ! #-1OK-but-with-cd: $(cd $w1DP && pwd -P)
...
ostmpdir1DP="$HOME/1tmp11/"     #- !reboot will delete them in /tmp/ ! here tmp/tr-stuff in case prj/... requires temporary accessible to OS/apps/..., also tempo 1RZ, 1m, ...
ostmpvar1DP="$HOME/1var11/"     #- !reboot will delete them in /tmp/ ! here all tmp-auto-files as caches/vars/... as pycache, pytestcache, ...
...
q_MntDP=${q_UppDP}/${q_MntDN}      ##--Evv-mnt-dirP,   so mountPoints.root.DirN for Evv-fixed-mounted-parts
q_MediasDP=${q_UppDP}/${q_MediasDN}   ##--Evv-media-dirP, so mountPoints.root.DirN for Evv-flexxible-mounted-medias (usb, NW,...)
...
#####  ==========  cmds/OSxxx/....:
te1cmd1a='xfce4-terminal'                        ##--terminal1-cmd1-a  to call a terminal (eg in mswin set to windowsterminal, ...
te1cmd1b="${te1cmd1a}  --disable-server"         ##--terminal1-cmd1-b  to call a terminal + some basic/evv-default args + some basic/evv-default args !
